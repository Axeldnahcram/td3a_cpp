...
def get_vectors_sklearn(n, h=10, dtype=numpy.float64):
    ...
    X = X.astype(numpy.float64)
    y = y.astype(numpy.float64)
    fct = RandomForestClassifier(n_estimators=10, max_depth=3, max_features=3)
    ctxs = [dict(X=X,
                 y=y,
...
for fct in [create_forest]:
    ctxs = get_vectors_cpp(fct, 20)

    df = DataFrame(list(measure_time_dim('create_forest(data, n_trees, max_depth, max_trees)', ctxs, verbose=1)))
    df['fct'] = fct.__name__
    dfs.append(df)
    print(df.tail(n=3))
